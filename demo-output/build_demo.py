...
OUTPUT_FULL = ROOT / "output.mp4"
OUTPUT_HERO = ROOT / "output-hero-silent.mp4"
BASE_URL = "https://mulesoft-claude-cursor-onboarding.vercel.app/"

MANIFEST = json.loads((ROOT / "scenes.json").read_text())
VOICE = MANIFEST.get("voice", "en-US-AndrewNeural")
SCENES = MANIFEST["scenes"]
HERO = MANIFEST["hero_silent"]
...
.accent-bar{position:absolute;top:0;left:0;right:0;height:6px;background:linear-gradient(90deg,var(--cloud-60),var(--orange-70));z-index:5}
...
def write_scene_html(scene: dict) -> None:
...
<img class="logo" src="../../mulesoft-logo.png" alt="MuleSoft"/>{inner}</body></html>"""
    (SCENES_DIR / f"{scene['id']}.html").write_text(html, encoding="utf-8")
...
async def capture_live():
    ...
    async with async_playwright() as p:
        ...
        async def shot(name, wait=700):
            await page.wait_for_timeout(wait)
            p = CAPTURES / name
            await page.screenshot(path=str(p))
            print("capture:", p)
        await page.goto(BASE_URL + "?editor=cursor", wait_until="networkidle")
        await shot("capture-tab-bar.png", 500)
        ...
        await shot("capture-scriptflow-arch.png", 800)
        await page.goto(BASE_URL + "?editor=salesforce", wait_until="networkidle")
        await shot("capture-salesforce-strategy.png", 600)
        await browser.close()

async def render_frames():
    from playwright.async_api import async_playwright
    FRAMES_DIR.mkdir(parents=True, exist_ok=True)
    waits = {"kinetic_pain": 1500, "kinetic_outcomes": 1200, "cta_inguide": 1300, "editor_tabs": 1100, "proof_highlight": 1400, "depth_tabs": 1200, "cta_steps": 1200}
    async with async_playwright() as p:
        browser = await p.chromium.launch()
        page = await browser.new_page(viewport={"width": 1920, "height": 1080})
        for scene in SCENES:
            sid = scene["id"]
            write_scene_html(scene)
            await page.goto((SCENES_DIR / f"{sid}.html").resolve().as_uri(), wait_until="networkidle")
            w = waits.get(scene.get("layout"), 950)
...
def edge_tts(text, mp3):
    edge = Path("/Users/mbusacca/Library/Python/3.9/bin/edge-tts")
    if edge.exists():
        run([str(edge), "--voice", VOICE, "--text", text, "--write-media", str(mp3)])
    else:
        run(["python3", "-m", "edge_tts", "--voice", VOICE, "--text", text, "--write-media", str(mp3)])
...
def make_clip(frame, audio, out, dur, ken_burns=False):
    if ken_burns:
        ...
        run(["ffmpeg","-y","-loop","1","-i",str(frame),"-vf",vf,"-t",f"{dur:.2f}",
             "-c:v","libx264","-pix_fmt","yuv420p","-an",str(tmp)])
        if audio:
            run(["ffmpeg","-y","-i",str(tmp),"-i",str(audio),"-c:v","copy","-c:a","aac",
                 "-b:a","192k","-shortest","-t",f"{dur:.2f}",str(out)])
            tmp.unlink(missing_ok=True)
        else:
            tmp.rename(out)
    else:
        vf = "scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2:color=0xEEF4FF"
...
def build_full():
    ...
    for scene in SCENES:
        ...
        (NARRATION_DIR / f"{sid}.txt").write_text(text + "\n")
        audio = AUDIO_DIR / f"{sid}.mp3"
        edge_tts(text, audio)
        dur = max(float(scene["duration"]), probe_duration(audio) + 0.35)
        make_clip(FRAMES_DIR / f"{sid}.png", audio, CLIPS_DIR / f"{sid}.mp4", dur,
                  ken_burns=scene.get("ken_burns", False))
        clips.append(CLIPS_DIR / f"{sid}.mp4")
    lst = ROOT / "concat-full.txt"
...
def build_hero_silent():
    ...
    print("Done hero:", OUTPUT_HERO, round(probe_duration(OUTPUT_HERO), 1), "s")

def main():
    for d in [SCENES_DIR, CAPTURES, NARRATION_DIR, FRAMES_DIR, AUDIO_DIR, CLIPS_DIR]:
